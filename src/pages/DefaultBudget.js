        ...
        .then(result => {
            ...
            if (result == '{"message":"The incoming token has expired"}' || result == '{"message":"Unauthorized"}') {
                ...
                <Container>
                    ...
                    <p>Here is your message: {result}</p>
                </Container>
                )
            }
            else {
                console.log("here's your default budget yo!", result)
                // get default budget ID
                // foreach budget in result
                    // if budget.isDefault == true
                        // var DefaultBudgetID = budget.ID
                setDefaultBudget(MyBudget => MyBudget)
                ...
                fetch("https://82u01p1v58.execute-api.us-east-1.amazonaws.com/Prod/categories", requestOptions)
                .then(response => response.json())
                .then(result => {
                    console.log(result)
                    if (result == '{"message":"The incoming token has expired"}' || result == '{"message":"Unauthorized"}') {
                        ...
                        setDefaultBudget(MyBudget => ({...MyBudget, ...result}))
                    }
